function initRsvpForm() {
  ...
  function renderResults(matches) {
    ...
    for (const match of matches) {
      ...
      btn.addEventListener("click", () => loadParty(match.party_id));
      li.appendChild(btn);
      searchResults.appendChild(li);
    }
  }
  ...
  rsvpForm.addEventListener("submit", async (e) => {
    ...
    for (const card of guestList.querySelectorAll(".guest-card")) {
      ...
      const attending = yesSelected;
      responses.push({ name, attending });
    }
    ...
    try {
      const resp = await fetch(`${RSVP_API_BASE}/rsvp`, {
        ...
        body: JSON.stringify({ party_id: selectedPartyId, responses }),
      });
      if (resp.ok) {
